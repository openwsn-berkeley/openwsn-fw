...
uint16_t neighbors_getLinkMetric(uint8_t index) {
    ...
    cellRadioSetting_t neighborRadio = neighbors_vars.neighbors[index].cellRadioSetting;
    ...
    if (neighbors_vars.neighbors[index].numTxACK==0) {
        ...
        rankIncreaseIntermediary = (3*rankIncreaseIntermediary * MINHOPRANKINCREASE * radioMinHopRankIncreaseFactor [neighborRadio]) / ((uint32_t)neighbors_vars.neighbors[index].numTxACK);
        rankIncreaseIntermediary = rankIncreaseIntermediary - ((uint32_t)(2 * MINHOPRANKINCREASE * radioMinHopRankIncreaseFactor [neighborRadio])<<10);
