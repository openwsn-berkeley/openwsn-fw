...
BEGIN_PACK
typedef struct {
    statusCtx_t statusSchedule;
    statusCtx_t statusBackOff;
} scheduleStatus_t;
END_PACK

//=========================== variables =======================================

scheduleVars_t schedule_vars;
scheduleStatus_t schedule_status_ctx;
...
static bool statusPrint_schedule(void);

static bool statusPrint_backoff(void);
...
void schedule_init(void) {
    ...
    memset(&schedule_vars, 0, sizeof(scheduleVars_t));
    ...
    schedule_status_ctx.statusSchedule.id = STATUS_SCHEDULE;
    schedule_status_ctx.statusSchedule.statusPrint_cb = statusPrint_schedule;
    openserial_appendStatusCtx(&schedule_status_ctx.statusSchedule);

    schedule_status_ctx.statusBackOff.id = STATUS_BACKOFF;
    schedule_status_ctx.statusBackOff.statusPrint_cb = statusPrint_backoff;
    openserial_appendStatusCtx(&schedule_status_ctx.statusBackOff);
}
...
statusPrint_* functions are used by the openserial module to continuously print
...
owerror_t schedule_addActiveSlot(
    ...
    if (entry_found == FALSE) { ENABLE_INTERRUPTS();
    ...
    if (schedule_vars.currentScheduleEntry == NULL) {
        ...
        while (1) {
            ...
            if (previousSlotWalker->slotOffset == slotContainer->slotOffset) {
                ...
                memset(&slotContainer->neighbor, 0, sizeof(open_addr_t));ENABLE_INTERRUPTS();
                return E_FAIL;
            }
...
bool schedule_isSlotOffsetAvailable(uint16_t slotOffset) {
    ...
    if (slotOffset >= schedule_vars.frameLength) { ENABLE_INTERRUPTS();
        return FALSE;
    }

    scheduleWalker = schedule_vars.currentScheduleEntry;
    do {
        if (slotOffset == scheduleWalker->slotOffset) { ENABLE_INTERRUPTS();
            return FALSE;
        }
        ...
        if (packetfunctions_sameAddress(&schedule_vars.scheduleBuf[i].neighbor, neighbor) == TRUE) {
            if (schedule_vars.scheduleBuf[i].numTx > MINIMAL_NUM_TX) {
                ...
                if (cellPDR < RELOCATE_PDRTHRES) {
                    ...
                    celllist->channeloffset = schedule_vars.scheduleBuf[i].channelOffset;ENABLE_INTERRUPTS();
                    return TRUE;
                }
            }
        }
...
bool schedule_getAutonomousTxRxCellUnicastNeighbor(open_addr_t *neighbor) {
    ...
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        if (
            ...
            memcpy(neighbor, &schedule_vars.scheduleBuf[i].neighbor, sizeof(open_addr_t));ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
...
bool schedule_hasNegotiatedTxCellToNonParent(open_addr_t *parentNeighbor, open_addr_t *nonParentNeighbor) {
    ...
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        if (
            ...
            memcpy(nonParentNeighbor, &schedule_vars.scheduleBuf[i].neighbor, sizeof(open_addr_t));ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
...
bool schedule_getAutonomousTxRxCellAnycast(uint16_t *slotoffset) {
    ...
    for (i = 0; i < MAXACTIVESLOTS; i++) {
        if (
            ...
            *slotoffset = schedule_vars.scheduleBuf[i].slotOffset;ENABLE_INTERRUPTS();
            return TRUE;
        }
    }
