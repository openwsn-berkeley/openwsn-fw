...
} timers_list[FRAGMENT_TX_MAX_PACKETS];
...
void fragment_test_timer_cb(opentimer_id_t id) {
   ...
   for (i=0;i<FRAGMENT_TX_MAX_PACKETS;i++)
      if (timer_list[i].timerId==id)
         pkt = timer_list[i].pkt;
...
owerror_t fragment_testSend(OpenQueueEntry_t* msg){
   ...
   DISABLE_INTERRUPTS();
   for (i=0;i<FRAGMENT_TX_MAX_PACKETS;i++)
      if (timer_list[i].timerId==-1) {
         timer_list[i].timer = opentimers_start(time, TIMER_ONESHOT, TIME_MS,
...
void fragment_init() {
   ...
   for (i=0;i<FRAGMENT_TX_MAX_PACKETS;i++) {
      timers_list[i].timer = -1;
      timers_list[i].pkt   = NULL;
   }
}
...
void fragment_retrieveHeader(OpenQueueEntry_t* msg) {
   ...
   // Initial fragment processed: process this one
      ...
      buffer->action(fragment_bufferIndex(buffer));
}
...
void fragment_disableTimer(FragmentQueueEntry_t* buffer) {
   if (buffer!=NULL && buffer->timerId!=-1) {
