...
void openbridge_receive(OpenQueueEntry_t* msg) {
   //local copy of the original payload
   uint8_t* payload;
   uint8_t  packet[130];
   uint8_t  length;

   //save the current packet to recover the original one after having pushed it to openserial
   payload  = msg->payload;
   length   = msg->length;
   memcpy(packet, msg->packet, sizeof(msg->packet));
   ...
   //recover the original payload after having pushed the data to openbridge
   msg->payload   = payload;
   msg->length    = length;
