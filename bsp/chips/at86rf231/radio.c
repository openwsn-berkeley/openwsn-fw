...
void radio_init(void) {
    ...
    // tell radio to fire interrupt on TRX_END and
    radio_spiWriteReg(RG_IRQ_MASK, (AT_IRQ_RX_START | AT_IRQ_TRX_END));

    // RX_START
    // de-assert the interrupt pin in case is high
    radio_spiReadReg(RG_IRQ_STATUS);

    // use chip antenna
    radio_spiWriteReg(RG_ANT_DIV, RADIO_CHIP_ANTENNA);

    // have the radio calculate CRC
...
void radio_getReceivedFrame(uint8_t *pBufRead, uint8_t *pLenRead, uint8_t maxBufLen, int8_t *pRssi, uint8_t *pLqi,
    ...
    *pRssi = -91 + temp_reg_value;
    ...
    // ===== packet
    radio_spiReadRxFifo(pBufRead, pLenRead, maxBufLen, pLqi);
}
...
void radio_spiReadRxFifo(uint8_t *pBufRead, uint8_t *pLenRead, uint8_t maxBufLen, uint8_t *pLqi) {
    ...
    *pLenRead = spi_rx_buffer[1];
    ...
    if (*pLenRead > 2 && *pLenRead <= 127) {
        ...
        // read packet
        spi_txrx(spi_tx_buffer, *pLenRead, SPI_BUFFER, pBufRead, 125, SPI_NOTFIRST, SPI_NOTLAST);
        ...
        *pLqi = spi_rx_buffer[2];
        ...
        spi_txrx(spi_tx_buffer, 1, SPI_BUFFER, spi_rx_buffer, sizeof(spi_rx_buffer), SPI_NOTFIRST, SPI_LAST);
    }
}
