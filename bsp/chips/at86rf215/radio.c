...
\author Jonathan Munoz <jonathan.munoz@inria.fr>, Octobre 2016.
*/
...
void radio_spiWriteReg(uint8_t* reg_addr, uint8_t reg_setting) {
   ...
   spi_tx_buffer[0] = (0x80 |(*reg_addr));        // turn addess in a 'reg write' address
   spi_tx_buffer[1] = *(reg_addr+1);
...
uint8_t radio_spiReadReg(uint8_t* reg_addr) {
   ...
   spi_tx_buffer[0] = *reg_addr;        // turn addess in a 'reg read' address
   spi_tx_buffer[1] = *(reg_addr+1); 
   spi_tx_buffer[2] = 0x00;             // send a no_operation command just to get the reg value
...
/** writes the tx buffer length in its register and the tx buffer to be sent **/

void radio_spiWriteTxFifo(uint8_t* bufToWrite, uint16_t  lenToWrite) {
   uint8_t spi_tx_buffer[4];
   uint8_t spi_rx_buffer[1+1+127];               // 1B SPI address, 1B length, max. 127B data
   
   spi_tx_buffer[0] = 0x83; 
   spi_tx_buffer[1] = 0x06;                       // SPI destination address for TXFIFO
   spi_tx_buffer[2] = lenToWrite/256;
   spi_tx_buffer[3] = lenToWrite%256;
   ...
   spi_tx_buffer[0] = 0x8a; 
   spi_tx_buffer[1] = 0x00;                       // SPI destination address for TXFIFO
