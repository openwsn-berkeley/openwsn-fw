...
port_INLINE uint8_t processIE_prependSyncIE(OpenQueueEntry_t* pkt){
   ...
   //keep a pointer to where the ASN should be.
   ...
   //subIE header
   packetfunctions_reserveHeaderSize(pkt, sizeof(MLME_IE_subHeader_t));
   //copy mlme sub-header               
   mlme_subHeader.length_subID_type = 
      sizeof(synch_IE_t) << IEEE802154E_DESC_LEN_SHORT_MLME_IE_SHIFT;
   mlme_subHeader.length_subID_type |= 
...
port_INLINE uint8_t processIE_prependFrameLinkIE(OpenQueueEntry_t* pkt){
   ...
   len=0;
   linkOption=0;
   slot=SCHEDULE_MINIMAL_6TISCH_ACTIVE_CELLS+SCHEDULE_MINIMAL_6TISCH_EB_CELLS;
...
port_INLINE uint8_t processIE_prependSixtopOpcodeIE(
   ...
   *((uint8_t*)(pkt->payload)) = uResCommandID;
   ...
   mlme_subHeader.length_subID_type  = 
      len << IEEE802154E_DESC_LEN_SHORT_MLME_IE_SHIFT;
   mlme_subHeader.length_subID_type |= 
      SIXTOP_MLME_RES_OPCODE_IE_SUBID << SIXTOP_MLME_RES_OPCODE_IE_SUBID_SHIFT|
      IEEE802154E_DESC_TYPE_SHORT;
   packetfunctions_reserveHeaderSize(pkt, sizeof(MLME_IE_subHeader_t));
   //little endian          
   pkt->payload[0] = mlme_subHeader.length_subID_type & 0xFF;
...
port_INLINE uint8_t processIE_prependSixtopBandwidthIE(
   OpenQueueEntry_t* pkt, 
   uint8_t numOfLinks, 
   uint8_t slotframeID){
   ...
   *((uint8_t*)(pkt->payload)) = slotframeID;
   ...
   mlme_subHeader.length_subID_type  = 
      len << IEEE802154E_DESC_LEN_SHORT_MLME_IE_SHIFT;
   mlme_subHeader.length_subID_type |= 
      (SIXTOP_MLME_RES_BANDWIDTH_IE_SUBID << 
         SIXTOP_MLME_RES_BANDWIDTH_IE_SUBID_SHIFT) |
      IEEE802154E_DESC_TYPE_SHORT;
   packetfunctions_reserveHeaderSize(pkt, sizeof(MLME_IE_subHeader_t));
   //little endian          
   pkt->payload[0] = mlme_subHeader.length_subID_type & 0xFF;
...
port_INLINE uint8_t processIE_prependSixtopGeneralSheduleIE(
   OpenQueueEntry_t* pkt,
   uint8_t type,
   uint8_t frameID,
   uint8_t flag,
   ...
   for(i=0;i<MAXSCHEDULEDCELLS;i++) {
      if(celllist[i].linkoptions != CELLTYPE_OFF){
         // cellobjects:2bytes slotOffset,2bytes channelOffset+ 1byte link_type
         packetfunctions_reserveHeaderSize(pkt,5); 
   ...
   *((uint8_t*)(pkt->payload)) = temp8b;
   ...
   mlme_subHeader.length_subID_type  = 
      len << IEEE802154E_DESC_LEN_SHORT_MLME_IE_SHIFT;
   mlme_subHeader.length_subID_type |= 
      (SIXTOP_MLME_RES_GENERAL_SCHEDULE_IE_SUBID << 
         SIXTOP_MLME_RES_GENERAL_SCHEDULE_IE_SUBID_SHIFT) | 
      IEEE802154E_DESC_TYPE_SHORT;
   packetfunctions_reserveHeaderSize(pkt, sizeof(MLME_IE_subHeader_t));
   //little endian          
   pkt->payload[0] = mlme_subHeader.length_subID_type & 0xFF;
...
port_INLINE void processIE_retrieveSyncIEcontent(
   OpenQueueEntry_t* pkt,uint8_t * ptr){
}
...
port_INLINE void processIE_retrieveTimeslotIE(
   OpenQueueEntry_t* pkt,
   uint8_t * ptr){
}

port_INLINE void processIE_retrieveSixtopLinkTypeIE(
   OpenQueueEntry_t* pkt,
   uint8_t * ptr){
}

port_INLINE void processIE_retrieveSixtopOpcodeIE(
   OpenQueueEntry_t* pkt,
   uint8_t * ptr,
   sixtop_opcode_subIE_t* opcodeInfo){
   uint8_t localptr;
...
port_INLINE void processIE_retrieveSixtopBandwidthIE(
   OpenQueueEntry_t* pkt,
   uint8_t * ptr,
   sixtop_bandwidth_subIE_t* bandwidthInfo){
   uint8_t localptr;
...
port_INLINE void processIE_retrieveSixtopGeneralSheduleIE(
   OpenQueueEntry_t* pkt,
   uint8_t * ptr,
   sixtop_generalschedule_subIE_t* scheduleInfo){
