...
enum {
   ...
   // l3
   ERR_UNEXPECTED_DAO                  = 0x09, // unexpected DAO (code position {0})
   ERR_UNSUPPORTED_ICMPV6_TYPE         = 0x0a, // unsupported ICMPv6 type {0} (code position {1})
   ...
   // l2b
   ERR_NEIGHBORS_FULL                  = 0x0f, // neighbors table is full (max number of neighbor is {0})
   ERR_NO_SENT_PACKET                  = 0x10, // there is no sent packet in queue
   ERR_NO_RECEIVED_PACKET              = 0x11, // there is no received packet in queue
   ERR_SCHEDULE_OVERFLOWN              = 0x12, // schedule overflown
   // l2a
   ERR_WRONG_CELLTYPE                  = 0x13, // wrong celltype {0} at slotOffset {1}
   ERR_IEEE154_UNSUPPORTED             = 0x14, // unsupported IEEE802.15.4 parameter {1} at location {0}
   ERR_DESYNCHRONIZED                  = 0x15, // got desynchronized at slotOffset {0}
   ERR_SYNCHRONIZED                    = 0x16, // synchronized at slotOffset {0}
   ERR_LARGE_TIMECORRECTION            = 0x17, // large timeCorrection of {0} ticks (code location {1})
   ERR_WRONG_STATE_IN_ENDFRAME_SYNC    = 0x18, // wrong state {0} in end of frame+sync
   ERR_WRONG_STATE_IN_STARTSLOT        = 0x19, // wrong state {0} in startSlot, at slotOffset {1}
   ERR_WRONG_STATE_IN_TIMERFIRES       = 0x1a, // wrong state {0} in timer fires, at slotOffset {1}
   ERR_WRONG_STATE_IN_NEWSLOT          = 0x1b, // wrong state {0} in start of frame, at slotOffset {1}
   ERR_WRONG_STATE_IN_ENDOFFRAME       = 0x1c, // wrong state {0} in end of frame, at slotOffset {1}
   ERR_MAXTXDATAPREPARE_OVERFLOW       = 0x1d, // maxTxDataPrepare overflows while at state {0} in slotOffset {1}
   ERR_MAXRXACKPREPARE_OVERFLOWS       = 0x1e, // maxRxAckPrepapare overflows while at state {0} in slotOffset {1}
   ERR_MAXRXDATAPREPARE_OVERFLOWS      = 0x1f, // maxRxDataPrepapre overflows while at state {0} in slotOffset {1}
   ERR_MAXTXACKPREPARE_OVERFLOWS       = 0x20, // maxTxAckPrepapre overflows while at state {0} in slotOffset {1}
   ERR_WDDATADURATION_OVERFLOWS        = 0x21, // wdDataDuration overflows while at state {0} in slotOffset {1}
   ERR_WDRADIO_OVERFLOWS               = 0x22, // wdRadio overflows while at state {0} in slotOffset {1}
   ERR_WDRADIOTX_OVERFLOWS             = 0x23, // wdRadioTx overflows while at state {0} in slotOffset {1}
   ERR_WDACKDURATION_OVERFLOWS         = 0x24, // wdAckDuration overflows while at state {0} in slotOffset {1}
   // general
   ERR_BUSY_SENDING                    = 0x25, // busy sending
   ERR_UNEXPECTED_SENDDONE             = 0x26, // sendDone for packet I didn't send
   ERR_NO_FREE_PACKET_BUFFER           = 0x27, // no free packet buffer (code location {0})
   ERR_FREEING_UNUSED                  = 0x28, // freeing unused memory
   ERR_FREEING_ERROR                   = 0x29, // freeing memory unsupported memory
   ERR_UNSUPPORTED_COMMAND             = 0x2a, // unsupported command {0}
   ERR_MSG_UNKNOWN_TYPE                = 0x2b, // unknown message type {0}
   ERR_WRONG_ADDR_TYPE                 = 0x2c, // wrong address type {0} (code location {1})
