/**
\brief k20-specific definition of the "bsp_timer" bsp module.

On k20, we use flextimer for the bsp_timer module.
...
void bsp_timer_reset() {
   // reset compare
   flextimer_reset();  // set cnt to initval
...
void bsp_timer_scheduleIn(PORT_TIMER_WIDTH delayTicks) {
   ...
   if (delayTicks<flextimer_getValue()-temp_last_compare_value) {
      ...
      // setting the interrupt flag triggers an interrupt
	   //bsp_timer_vars.cb();
	   flextimer_schedule(flextimer_getValue()+1);
   } else {
      flextimer_schedule(newCompareValue);
   }
}
