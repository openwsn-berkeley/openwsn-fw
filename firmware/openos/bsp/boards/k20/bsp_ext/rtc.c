...
void rtc_init(uint32 seconds, uint32 alarm, uint8 c_interval, uint8 c_value, uint8 interrupt) 
{
  ...
  RTC_IER = RTC_IER_TIIE_MASK	//time invalid interrupt enable
		| RTC_IER_TOIE_MASK		//time overflow interrupt enable 
		| RTC_IER_TAIE_MASK;	//time alarm interrupt enable
  
  //RTC_IER |= RTC_IER_TSIE_MASK;	//time second interrupt enable
...
{   
   ...
   else if((RTC_SR & RTC_SR_TAF_MASK) == 0x04)
   {
   	   ...
   	   RTC_TAR += 60;// Write new alarm value, to generate an alarm every 60s
   }	
   else
   {
