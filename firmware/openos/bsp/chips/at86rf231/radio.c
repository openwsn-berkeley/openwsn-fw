...
void radio_init() {
   ...
   //PORT_PIN_RADIO_SLP_TR_CNTL_HIGH();  //SLP_TR = H, radio sleep
   //while((radio_spiReadReg(RG_TRX_STATUS) == 0x1F));   //Wait for sleep
...
//   radio_rfOff();
//      uint8_t reg = radio_spiReadReg(RG_TRX_STATUS) & 0x1F;
//      int p = reg;
//      radio_rfOn();
//      reg = radio_spiReadReg(RG_TRX_STATUS) & 0x1F;
//      p = reg;
}
...
void radio_configReg() {
   ...
   radio_spiReadReg(RG_IRQ_STATUS);                       // deassert the interrupt pin in case is high
   radio_spiWriteReg(RG_ANT_DIV, RADIO_CHIP_ANTENNA);     // use chip antenna
   radio_spiWriteReg(RG_TRX_CTRL_1, 0x20);  
}
...
void radio_rfOn() {
   if (radio_vars.state == RADIOSTATE_RFOFF)  {
      PORT_PIN_RADIO_SLP_TR_CNTL_LOW(); //SLP_TR = L, end radio sleep
      while((radio_spiReadReg(RG_TRX_STATUS) & 0x1F) != TRX_OFF); //Wait for awake
   }
}

void radio_rfOff() {
  int reg = (radio_spiReadReg(RG_TRX_STATUS) & 0x1F);
  if ((radio_vars.state != RADIOSTATE_RFOFF) && (reg != SLEEP) && (reg != P_ON))
  { 
  ...
  radio_vars.state = RADIOSTATE_TURNING_OFF;
   ...
   PORT_PIN_RADIO_SLP_TR_CNTL_HIGH(); //SLP_TR = H, radio sleep, In SLEEP state register not accessible
   leds_radio_off();
...
void radio_txEnable() {
   ...
   //while((radio_spiReadReg(RG_TRX_STATUS) & 0x1F) != TRX_OFF);   //Wait for awake
//  radio_rfOn(); 
//  if((radio_spiReadReg(RG_TRX_STATUS) & 0x1F) != TRX_OFF) { 
//       int i = radio_spiReadReg(RG_TRX_STATUS) & 0x1F;
//
//    i++;
//  }
  radio_spiWriteReg(RG_IRQ_MASK,
