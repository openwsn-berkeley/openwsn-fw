...
#define SIXTOP_REMOVE_UNUSED_CELLS        // a cell is removed when it is not used for a sufficient long time
#define SIXTOP_CELL_TIMEOUT_RX         (3 * TIMER_DAO_TIMEOUT)        //ms before a cell in RX is considered unused if nothing is received
#define SIXTOP_CELL_TIMEOUT_TX         (2 * TIMER_DAO_TIMEOUT)        //ms before a cell in TX is considered unused if nothing is transmitted

//OPENBRIDGE
#define OPENCOAP_DROPREP_DAGROOT          //CoAP drops the replies generated by the application if we are the DAGroot

//Forwarded packets have a timeout in the queue
#define TIMEOUT_FORWARDING

//TIMESLOT DURATION in MS
#define TSLOTDURATION_MS    (TsSlotDuration / PORT_TICS_PER_MS)
...
enum {
   ...
   ERR_SIXTOP_WRONG_HANDLER            = 0x44, // sixtop requires a non null handler
   ERR_SIXTOP_TIMEOUT                  = 0x45, // a timeout has been fired. We have to flush 6top packets (we have the state {0}, timeout {1})
   ERR_OPENQUEUE_TIMEOUT               = 0x46, // a packet has been removed from the queue (owner {0], creator {1})
   ERR_BAD_TRACKID                     = 0x47, // the best effort track cannot have an owner (here type {0} and addr {1})
   ERR_OPENQUEUE_OVERSIZE              = 0x48, // not enough space in openqueue for non prioritar packets
   ERR_IEEE154_BADCRC                  = 0x49, // received frame with invalid CRC
   ERR_IEEE154_INVALIDHEADER           = 0x4a, // invalid IEEE 802.15.4e headers
   ERR_OPENSERIAL_BUFFER_OVERFLOW      = 0x4b, // output buffer overflow in openserial (available space {0}, towrite {1})
   ERR_OPENQUEUE_BUFFER_OVERFLOW       = 0x4c, // openqueue overflow: too many packets (space left {0}, reserved for management {1})
   ERR_SIXTOP_TOOMANY_CELLS            = 0x4d, // too many cells to reserve in the sixtop request (asked {0}, max {1})
   ERR_SIXTOP_MULTIPLE_TRACKS          = 0x4e, // the cells from the same LinkRem are associated with different tracks
   ERR_OPENQUEUE_DESYNC                = 0x4f, // a packet cannot be allocated for component {0} when the node is not synchronized
   ...
   ERR_GENERIC                         = 0x52, // generic error {0} {1}
   ERR_UNKNOWN                         = 0x53, // unknown error, location {0}, arg {1}
};
