...
This was done on 2014-07-18 12:02:33.186290.
*/
/**
...
void security_outgoingFrame(OpenMote* 			self,
	...
	//search for a key
	match = keyDescriptorLookup(self,
								keyIdMode,
								keySource,
								keyIndex,
								nextHop,
...
//		break;
 ...
 packetfunctions_writeAddress(self, msg,keySource,OW_LITTLE_ENDIAN);
		break;
	default:
		return;
	}
...
//		if(msg->l2_frameType == IEEE154_TYPE_ACK){
//		uint16_t diff;
//		diff = ieee154e_asnDiff(self, &init);
//
...
void retrieve_AuxiliarySecurityHeader(OpenMote* 			 self,
									  OpenQueueEntry_t*      msg,
...
void security_incomingFrame(OpenMote* 			   self,
							OpenQueueEntry_t*      msg){
	uint8_t match;
 ...
 idmanager_getMyID(self, ADDR_PANID),
	...
	match = deviceDescriptorLookup(self,
								   &Address,
								   idmanager_getMyID(self, ADDR_PANID),
						   	   	   keypoint);
...
uint8_t deviceDescriptorLookup(OpenMote*    self,
							   open_addr_t* Address,
							   open_addr_t* PANId,
	...
	for(i=0; i<MAXNUMNEIGHBORS; i++){
		...
		if(( packetfunctions_sameAddress(self, Address,keydescr->DeviceTable->DeviceDescriptorEntry[i].deviceAddress)== TRUE)
			&&
			( packetfunctions_sameAddress(self, PANId, MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId))
				){
			return i;
		}
	}
...
uint8_t keyDescriptorLookup(OpenMote*       self,
							uint8_t  		KeyIdMode,
					     	open_addr_t*	keySource,
						 	uint8_t 		KeyIndex,
	...
	if(KeyIdMode == 0){
		...
		for(i=0; i<MAXNUMKEYS; i++ ){
			if( packetfunctions_sameAddress(self, DeviceAddress,MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.Address)
					&& packetfunctions_sameAddress(self, panID,MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId)){
				return i;
			}
		}
...
//							&& packetfunctions_sameAddress(self, keySource,m_macDefaultKeySource)){
//							return i;
//							}
//						}
//			}
...
//		}
	...
	if (KeyIdMode == 3){
		...
		for(i=0; i<MAXNUMKEYS; i++ ){
				...
				if( packetfunctions_sameAddress(self, keySource,MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.KeySource)
						&& packetfunctions_sameAddress(self, panID, MacKeyTable.KeyDescriptorElement[i].KeyIdLookupList.PANId)
						){
				return i;
				}
...
void coordinator_init(OpenMote* self){

	open_addr_t*  my;
	my = idmanager_getMyID(self, ADDR_64B);
	//my = idmanager_getMyID(self, ADDR_16B);
	...
	MacKeyTable.KeyDescriptorElement[0].KeyIdLookupList.PANId = *( idmanager_getMyID(self, ADDR_PANID));

	MacKeyTable.KeyDescriptorElement[0].KeyUsageList[1].FrameType = IEEE154_TYPE_DATA;
	MacKeyTable.KeyDescriptorElement[0].KeyUsageList[0].FrameType = IEEE154_TYPE_ACK;
