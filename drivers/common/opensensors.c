...
void opensensors_register(uint8_t sensorType) {
    if (sensors_is_present(sensorType)) {
        ...
        opensensors_vars.opensensors_resource[opensensors_vars.numSensors].callbackRead = sensors_getCallbackRead(
                sensorType);
        opensensors_vars.opensensors_resource[opensensors_vars.numSensors].callbackConvert = sensors_getCallbackConvert(
                sensorType);
