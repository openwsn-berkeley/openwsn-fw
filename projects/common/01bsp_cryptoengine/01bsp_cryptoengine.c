...
{
   uint8_t key[16];
   uint8_t m[16 + 1 * 16]; // IV + message
   uint8_t len_m;
   uint8_t expected_ciphertext[1 * 16];
} aes_cbc_suite_t;
...
static owerror_t run_aes_cbc_suite(aes_cbc_suite_t* suite, uint8_t test_suite_len) {
   ...
   for(i = 0; i < test_suite_len; i++) {
	   if(CRYPTO_ENGINE.aes_cbc_enc_raw(suite[i].m,
                                       suite[i].len_m,
                                       suite[i].key) == E_SUCCESS) {
         
         if(memcmp(&suite[i].m[16], suite[i].expected_ciphertext, suite[i].len_m - 16) == 0) {
            success++;
         }
      }
   }
