...
while True:
    ...
    if rawFrame[-3:]==[0xff]*3 and len(rawFrame)>=9:
        ...
        (rxpk_len,rxpk_num,rxpk_src,rxpk_rssi,rxpk_lqi,rxpk_crc, rxpk_freq_offset) = \
            struct.unpack('>BBBbBBb', ''.join([chr(b) for b in rawFrame_decoded[-10:-3]]))
            
        # debug info
        output = 'src={0:<3} len={1:<3} num={2:<3} rssi={3:<4} lqi={4:<3} crc={5} freq_offset={6:<4}'.format(
            rxpk_src,
            rxpk_len,
            rxpk_num,
            rxpk_rssi,
