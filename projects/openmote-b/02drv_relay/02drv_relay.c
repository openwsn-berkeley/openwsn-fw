...
int mote_main(void) {
    ...
    while (1) {
        ...
        app_vars.uart_txFrame[i++] = app_vars.rxpk_src;  // packet source EUI
...
void cb_endFrame(PORT_TIMER_WIDTH timestamp) {
    ...
    uint8_t rxpk_len = (uint8_t) app_vars.rxpk_len;
    ...
    // if I am at neither ends of the network, just keep relaying 
    ...
    if (tx_started ==0){
        tx_started = 1;
        radio_rfOff();
        radio_loadPacket(app_vars.rxpk_buf,app_vars.rxpk_len);
        radio_txEnable();
        radio_txNow();
    }else{
        // I am just done transmitting, switch to rx mode
        tx_started = 0;
        radio_rfOff();
        radio_rxEnable();
        radio_rxNow(); 
