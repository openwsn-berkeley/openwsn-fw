...
#include "03oos_mercator_subghz.h"
#include <stddef.h>
...
void serial_rx_REQ_TX(void) {
   ...
   memset(&mercator_vars.rfbuftx[offsetof(RF_PACKET_ht, txfillbyte)], req->txfillbyte,
          mercator_vars.txpk_len - offsetof(RF_PACKET_ht, txfillbyte) - LENGTH_CRC_SUBGHZ);

   // prepare radio
   radio_subghz_rfOn();
   radio_subghz_setFrequency(req->channel_spacing,req->frequency_0,req->channel);
...
void cb_sendPacket(opentimers_id_t id){
   ...
   radio_subghz_loadPacket(mercator_vars.rfbuftx, mercator_vars.txpk_len);
